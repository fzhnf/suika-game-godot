...
@onready var score_markergame_result: Marker2D = $ScoreMarker
...
func _ready() -> void:
	...
	SignalBus.restart.connect(game_restart)
	pass
...
func create_fruit(pos: Vector2) -> RigidBody2D:
	...
	new_fruit.size = randi_range(1,1)
	add_child(new_fruit)
	return new_fruit
...
func get_fruit_x()-> float:
	...
	if current_fruit != null:
		left_marker_x += current_fruit.get_node('CollisionShape2D').get_shape().get_radius()
		right_marker_x -= current_fruit.get_node('CollisionShape2D').get_shape().get_radius()
	return clamp (
...
func detect_game_over(body: RigidBody2D) -> void:
	var height: float = body.global_position.y - body.get_node('CollisionShape2D').get_shape().get_radius()
	if not height < top_marker.global_position.y:
		return
